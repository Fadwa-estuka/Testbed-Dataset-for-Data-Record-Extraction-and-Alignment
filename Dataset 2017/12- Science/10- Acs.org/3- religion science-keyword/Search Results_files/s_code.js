...
s.visitorNamespace="americanchemicalsociety"
...
s.linkDownloadFileTypes="dmg,exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx,apk,ipa"
// PROD   
s.linkInternalFilters="acs.org,account.acs.org,acs.cn.com,acspressblog.com,acswebcontent.acs.org,acswebcontent.acs.org/iyckit/,acswebinars.org,bytesizescience.com,careerfair.acs.org,ccedlookup.acs.org,communities.acs.org,congressweb.com,demographics.acs.org,dgr.rints.com,donate.acs.org,edmc.acs.org,emailpref.acs.org,es.acs.org,eweb.acs.org,eweb1.acs.org,ewebdev.acs.org,gcande.org,getexperience.acs.org,getexperience.dreamhosters.com,giving.acs.org,global.acs.org,gradstudent.acs.org,highschoolenergy.acs.org,info.acs.org,inquiryinaction.org,join.acs.org,join.teachchemistry.org,membernumberlookup.acs.org,middleschoolchemistry.com,my.acs.org,ncwlookup.acs.org,portal.acs.org,presentations.acs.org,renew.acs.org,renew.teachchemistry.org,search.acs.org,sites.acs.org,sso.acs.org,ssotest.acs.org,store.acs.org,strategy.acs.org,teachchemistry.acs.org,teachchemistry.org/,Undergrad.acs.org,webapplications.acs.org,worldwide.acs.org,www.acs.org,xpressreg.net/register/acsf085/,zh-cn.acs.org";
...
var s_carouselIDs="";
var s_carouselFinish=false;
var s_carouselTitles="";
var s_tabsClicked="";
var s_activeTabID="";
var s_activeTabTitle="";
var s_activeTabPosition="";
...
s.campaign=s.getQueryParam('sc'); 
    ...
    if (siteURL == "presentations.acs.org") { // Only run this function if the URL is presentations.acs.org
...
/* Campaign Pathing */
if(s.pageName) {
    if(""!=s.getAndPersistValue(s.campaign,'s_persist_campaign',0)) 
        s.prop24='D="'+s.getAndPersistValue(s.campaign,'s_persist_campaign',0)+'"+":"+pageName';
    else
        s.prop24='D="NONE:"+pageName';
}
 
/* Determine Daylight Savings Time start and end */
var dst_start = new Date;var dst_end = new Date;
dst_start.setMonth(2); dst_start.setDate(14);
var day = dst_start.getDay();
dst_start.setDate(14-day); // second Sunday in March
dst_end.setMonth(10); dst_end.setDate(7);
day = dst_end.getDay();
...
var search_term = "";
if(s.pageName && s.pageName.indexOf('Search:index.htm') > -1) /* Check search page*/
{
    var s_searchType = '';
    ...
    var s_start = parseInt(s.getQueryParam('start'),10);
    if (!s_start) s_start = 0;  
    var s_pagesize = parseInt(s.getQueryParam('num'),10);
    ...
    search_term = s.getQueryParam("q").trim().toLowerCase();
    if(search_term) {
        if(s_pagenum==1) {
    ...
    if(s.prop13) {
        ...
        var s_contentfilter=s.getQueryParam("partialfields");
...
if(!search_term && s_searchterm)
    s.getPreviousValue(s_searchterm,'s_searchterm','');
...
var s_onebox = "";
if(location.pathname.toLowerCase().indexOf('/search')!=-1 && document.activeElement && document.activeElement.tagName && document.activeElement.tagName.toLowerCase()=='a'){ /*Check link clicks*/
    if(document.activeElement.parentNode && document.activeElement.parentNode.parentNode && document.activeElement.parentNode.parentNode.parentNode && document.activeElement.parentNode.parentNode.parentNode.parentNode && document.activeElement.parentNode.parentNode.parentNode.parentNode.parentNode && document.activeElement.parentNode.parentNode.parentNode.parentNode.parentNode.className=="elements-onebox"){
...
if(location.pathname.toLowerCase().indexOf('/search')!=-1 && document.activeElement && document.activeElement.getAttribute('ctype')) {
    ...
    s_link_ctype=s.getPreviousValue('','gpv_linkctype',0);
if(s_link_ctype && s_link_ctype!="no value") /* Previous link click had a valid ctype */
{
    if(s_link_ctype=="c") {
        ...
        s.prop48='D="ACS_search:ctype="+"'+s_link_ctype+'"+"|"+c34';
    s.prop49='D="'+s_searchterm+'|"+pageName';
}
    ...
    if (obj.href) {
        ...
        if (s.trackExternalLinks == false) {
            ...
            if (isExitLink) {
                ...
                s.linkTrackVars = "events,prop70,eVar70";
...
function trackDownloads(){
    ...
    for(var i=0;i < myLinks.length;i++) {
        if(myLinks[i].href.indexOf(".pdf") > -1 || myLinks[i].href.indexOf(".zip") > -1
    || myLinks[i].href.indexOf(".exe") > -1 || myLinks[i].href.indexOf(".wav") > -1
    || myLinks[i].href.indexOf(".mp3") > -1 || myLinks[i].href.indexOf(".mov") > -1
    || myLinks[i].href.indexOf(".mpg") > -1 || myLinks[i].href.indexOf(".avi") > -1
    || myLinks[i].href.indexOf(".wmv") > -1 || myLinks[i].href.indexOf(".doc") > -1
    || myLinks[i].href.indexOf(".docx") > -1 || myLinks[i].href.indexOf(".xls") > -1
    || myLinks[i].href.indexOf(".xlsx") > -1 || myLinks[i].href.indexOf(".ppt") > -1
    || myLinks[i].href.indexOf(".pptx") > -1){
            var thehref = linkName = '';
            thehref = myLinks[i].href.toString();
            linkName = myLinks[i].innerHTML;
            var f =function(thehref,linkName){return function(){trackD(thehref,linkName);};}(thehref,linkName)
            myLinks[i].onclick = f;
        }
    }
}
window.onload = trackDownloads;
 
/************************** Flowplayer Video Tracking *************************/
function trackFlowPlayerVideo(){
    if(typeof($f) != 'undefined'){
        if(typeof($f()) != 'undefined'){
            $f().onStart(function(){
                s.Media.open((this.getClip().url),(this.getClip().duration),"Flow Player");
                s.Media.play((this.getClip().url),0);
                }); 

            $f().onPause(function(){
                s.Media.stop(this.getClip().url,(this.getTime()));
                });


            $f().onStop(function(){
                s.Media.stop(this.getClip().url,(this.getTime()));
                    s.Media.close(this.getClip().url);
                });

            $f().onResume(function(){
                s.Media.play(this.getClip().url,(this.getTime()));
                });

            $f().onBeforeSeek(function(){
                s.Media.stop(this.getClip().url,(this.getTime()));
                });
...
window.onload = trackFlowPlayerVideo;
/************************** END Flowplayer Video Tracking  *************************/
 
 
/************************** LINK FUNCTIONS *************************/
/* These functions are standalone and can be called in any onClick, or*/
/* within a navigatetoURL function call in Flash*/
 
/* Set whenever user clicks "Save preferences" on the My Preferences dialog box to change text size or page width*/
s_saveSizePreferences = function(textSize,pageWidth) {
    s.linkTrackVars='eVar49,prop8,prop9,prop10,eVar8,eVar9,eVar10';  
    s.eVar49=textSize+':'+pageWidth;
 
    var link_name = 'text_size_change';
    s.tl(true,'o',link_name);
}
 
/* Fired on all slide transitions of carousel.  If tab position is not known, pass empty string ''*/
s_tabImpression = function(tabID,tabTitle,tabPosition) {
    s_activeTabID=tabID;
    s_activeTabTitle=tabTitle;
    s_activeTabPosition=tabPosition;
    if(!s_carouselFinish && (!s_carouselTitles || s_carouselTitles.indexOf(tabTitle+',')==-1)) { 
        s_carouselIDs+=tabID+',';
        s_carouselTitles+=tabTitle+',';
    }
    else if(!s_carouselFinish && s_carouselTitles && s_carouselTitles.indexOf(tabTitle+',')==0) {
        s_carouselFinish=true;
        s.linkTrackVars='prop35,eVar35,prop36,eVar36,prop6,eVar6,events,prop8,prop9,prop10,eVar8,eVar9,eVar10';  
        s.linkTrackEvents='event8';
        s.events='event8';
        s.prop6='Tabs_shown:'+s_carouselIDs;
        s.prop36='Tabs_shown:'+s_carouselTitles;
        s.eVar6=s_carouselIDs;
        s.eVar36=s_carouselTitles;
        if(!tabPosition) tabPosition='Unknown';
        s.eVar35='Homepage_carousel';
        s.prop35='Tabs_shown:Homepage_carousel_finish';
 
        var link_name = 'Homepage_carousel_finish';
        s.tl(true,'o',link_name);
    }
}
 
/* Fired whenever a visitor clicks on a homepage tab */
s_tabMenuClick = function(tabID,tabTitle,tabPosition) {
    if(!s_tabsClicked || s_tabsClicked.indexOf(tabTitle+',')==-1) {
        s_tabsClicked+=tabTitle+',';
        //s_activeTab=tabID;
        s.linkTrackVars='prop35,eVar35,prop36,eVar36,prop6,eVar6,events,prop8,prop9,prop10,eVar8,eVar9,eVar10';  
        s.linkTrackEvents='event26';
        s.events='event26';
        s.prop6='Tab_click:'+tabID;
        s.prop36='Tab_click:'+tabTitle;
        if(!tabPosition) tabPosition='Unknown';
        s.eVar35=tabPosition;
        s.prop35='Tab_click:'+tabPosition;
        s.eVar6=tabID+'';
        s.eVar36=tabTitle+'';
         
        var link_name = 'Tab_click:'+tabTitle;
        s.tl(true,'o',link_name);
    }
}
 
/* Fired whenever a visitor clicks on a carousel menu item */
s_slideMenuClick = function(tabID, tabTitle, tabPosition) {
    s_activeTabID = tabID;
    s_activeTabTitle = tabTitle;
    s_activeTabPosition = tabPosition;
     
    s_tabMenuClick(tabID, tabTitle, tabPosition);
}
 
/* Fired whenever a visitor clicks on a link within the homepage Flash carousel */
s_slideLinkClick = function(urlClicked) {
    if (!s_activeTabID || s_activeTabID == "") s_activeTabID = 'Unknown';
    if (!s_activeTabTitle || s_activeTabTitle == "") s_activeTabTitle = 'Unknown';
    if (!s_activeTabPosition || s_activeTabPosition == "") s_activeTabPosition = 'Unknown';
 
    s.linkTrackVars = 'prop35,eVar35,prop36,eVar36,prop6,eVar6,events,prop8,prop9,prop10,eVar8,eVar9,eVar10,prop41';
    s.linkTrackEvents = 'event27';
    s.events = 'event27';
    s.prop6 = 'Tab_content_click:' + s_activeTabID;
    s.prop36 = 'Tab_content_click:' + s_activeTabTitle;
    s.eVar35 = s_activeTabPosition + '';
    s.prop35 = 'Tab_content_click:' + s_activeTabPosition;
    s.eVar6 = s_activeTabID + '';
    s.eVar36 = s_activeTabTitle + '';
    s.prop41 = urlClicked;
 
    var link_name = 'Tab_content_click:' + s_activeTabTitle;
    s.tl(true, 'o', link_name);
}
 
/* Fired whenever a visitor clicks on a link within the homepage tab */
s_tabContentClick = function(tabID, tabTitle, tabPosition) {
    s.linkTrackVars = 'prop35,eVar35,prop36,eVar36,prop6,eVar6,events,prop8,prop9,prop10,eVar8,eVar9,eVar10,prop41';
    s.linkTrackEvents = 'event27';
    s.events = 'event27';
    s.prop6 = 'Tab_content_click:' + tabID;
    s.prop36 = 'Tab_content_click:' + tabTitle;
    if (!tabPosition) tabPosition = 'Unknown';
    s.eVar35 = tabPosition;
    s.prop35 = 'Tab_content_click:' + tabPosition;
    s.eVar6 = tabID + '';
    s.eVar36 = tabTitle + '';
 
    var link_name = 'Tab_content_click:' + tabTitle;
...
/*Added videoPlayerType1 to a.contentType in media module. Splits fixed web and mobile players. Set type variable on page or within player*/
...
+"Data,x;c['a.contentType']=videoPlayerType1;c[ns+'name']=i.n;c[ns+'playerName']=i.p;if(i.l>0){c[ns+'length']=i.l;}c[ns+'timePlayed']=Math.floor(i.ts);if(!i.vt){c[ns+'view']=true;pe='m_s';i.vt=1}if(i.sx){c[ns"
