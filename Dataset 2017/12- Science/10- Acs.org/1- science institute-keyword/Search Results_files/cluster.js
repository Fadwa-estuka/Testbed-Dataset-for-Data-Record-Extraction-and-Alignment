...
function cs_createAnchorCode(url, text) {
  var encodedurl = url.replace(/'/g, uri_encodeOne_("'"));
...
function cs_showLabels(maxLabels, srchArgs, clusters, documents) {
  var searchUri;
  // Compose the search URI using the newly added search root path prefix finder
  // functions for supporting embedded mode (if available).
  if (window['GSA_getSearchRootPathPrefix']) {
    searchUri = window['GSA_getSearchRootPathPrefix']() + '?' + srchArgs;
  } else {
    searchUri = "/search?" + srchArgs;
  }
...
function cs_loadClusters(srchArgs, render) {
  ...
  // If GSA search results is being accessed in embedded mode then
  // send request via the proxy URL for resource access.
  if (window['GSA_isEmbeddedMode'] && window['GSA_isEmbeddedMode']()) {
