...
;(function(W, D, N, nm) {
	...
	HtmlNotice[_pr] = {
		...
		, showModal: function () {
			if (!this.getPop()) {
				var frag = HtmlNotice.createHtml(
					'<div class="sg-b-p">' + 
					'<div class="sg-b-p-h">' + 
					'<button type="button" class="sg-b-p-c">&times;</button>' +
					'</div>' +
					'<div class="sg-b-p-t"></div>' + 
					'<iframe src="' + this.url + '" frameborder="0" class="sg-b-p-i" style="display:none"></iframe>' +
					'</div>' +
		...
		, getPar: function () {
			return (String(this.config.text)[_l] ? '<p class="sg-b-t">' + this.config.text + '</p>' : '') + 
		...
		, headerHtml: function () {
			return HtmlNotice.createHtml(
				'<div class="sg-b-h sg-b-bar" '+ this.fixedHtmlTop +'>' + 
				'<div class="sg-d-h"><a class="sg-js-d" href="Javascript:void(0);">&times;</a></div>' +
				this.getPar() + 
				'</div>'
			);
		}
		...
		, footerHtml: function () {
			return HtmlNotice.createHtml(
				'<div class="sg-b-f sg-b-bar" '+ this.fixedHtmlBottom +'>' + 
				'<div class="sg-d-h"><a class="sg-js-d" href="Javascript:void(0);">X</a></div>' +
				this.getPar() +
				'</div>'
			);
		}
		...
		, addInline: function (e) {
			...
			l && _aE(l, 'click', function() {
				...
				frag = HtmlNotice.createHtml('<iframe src="' + th.url + '" frameborder="0" class="sg-b-p-i"></iframe>');
