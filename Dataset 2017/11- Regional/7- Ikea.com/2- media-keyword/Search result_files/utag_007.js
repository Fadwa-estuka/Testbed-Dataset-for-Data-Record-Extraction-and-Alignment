//tealium universal tag - utag.loader ut4.39.201612291302, Copyright 2016 Tealium.com Inc. All Rights Reserved. 

if(typeof utag_err=='undefined')var utag_err=[];window._tealium_old_error=window._tealium_old_error || window.onerror || function(){};window.onerror=function(m,u,l){if(typeof u !== 'undefined' && u.indexOf('/utag.')>0 && utag_err.length < 5)utag_err.push({e:m,s:u,l:l,t:'js'});window._tealium_old_error(m,u,l)};
var utag_condload=false;try{(function(){function ul(src,a,b){a=document;b=a.createElement('script');b.language='javascript';b.type='text/javascript';b.src=src;a.getElementsByTagName('head')[0].appendChild(b)};if((""+document.cookie).match("utag_env_ikea_ca-main=([^\S;]*)")){if(RegExp.$1.indexOf("/prod/") === -1) {ul(RegExp.$1);utag_condload=true;__tealium_default_path='//tags.tiqcdn.com/utag/ikea/ca-main/prod/';}}})();}catch(e){};try{
/*
 ...
 * 
 * extension: product-restapi-localizer
 * 
 * purpose: multi-language markets has to deal with the fact that some data sources
 * will have a different language mix, depending on the visitor preference. by using
 * this extension, the stakeholder can choose a language code and if the visitor chooses
 * a different code, this extension will use the rest api service and default the
 * collected values back to the preferred language.
 * 
 * scope: pre-loader (!important)
 */
if (!window.irwstats_locale) { return true; } // old web only
...
window.TEALIUM = window.TEALIUM || {};
window.TEALIUM.log = window.TEALIUM.log || function(type, msg){ try { var message = "TEALIUM LOG::"; if (type === "error") { message += "Something unexpected happened."; message += "Please contact kevin.faurholt@tealium.com with the below message to get it fixed:\n\t" + msg.stack;} else { message += msg;} var logger = window.console[type] || window.console.log; logger.call(window.console, message); } catch(e) {} };
...
// override global request method
window.TEALIUM.ajax.localrequest = function(partNo, country, preferred_language, callback, callbackindex) {
  
  var data = {}, env = window.irwstats_locale.split("_");
  ...
  if (new RegExp(env[0], "i").test(preferred_language)) {
    
    data.preferred_language = preferred_language;
    data.ajax_status = 304;
    data.ajax_message = "no need for reverse lookup, all good";
    
    // nothing to do, preferred language already chosen
    return window.TEALIUM.ajax.localresponse.apply(this, [data, callback]);
  }
  
  /* http://www.ikea.com/DE/de/iows/catalog/items/art/20238223?method=GET&version=v2 */
  var url = "http://www.ikea.com/" + country + "/" + preferred_language + "/iows/catalog/items/art/" + partNo + "?method=GET&version=v2";
  ...
  new Ajax.Request(url, {
    ...
    "onSuccess": function(oXHR, oJson) {
      ...
      data.ajax_status = oXHR.status;      
      data.ajax_message = oXHR.statusText;
      ...
      var doc = oXHR.responseXML.documentElement;

      Object.extend(data, window.TEALIUM.ajax.XML2JSON(doc));
...
window.TEALIUM.ajax.localresponse = function(data, callback) {
  
  // post ajax processing here
  var response = {};
  response.preferred_language = data.preferred_language;
  response.ajax_status = data.ajax_status;
  response.ajax_message = data.ajax_message;
  
  if (data.callbackindex !== undefined) {
    
    response.callbackindex = data.callbackindex;
  }
  
  if (response.ajax_status === 200) {

    //console.log("response data: ", data);
    //<ProductName>FALSTER</ProductName>
    //<ProductTypeName>Stuhl</ProductTypeName>
    //<ValidDesignText>schwarz, braun</ValidDesignText>
    //<RetailItemCommPrice>
    response.product_name = data.ProductName.toUpperCase();    
    response.product_description = data.ProductTypeName.toLowerCase();
    ...
    //response.product_description += data.ValidDesignText ? ", " + data.ValidDesignText.toLowerCase() : "";    
  }
...
window.TEALIUM.localize = function(callback) {
  
  var localdata = {},
    restapi_partnumbers = [],
    event_module = window.TEALIUM.datalayer.event,
    module = null;

  // page datalayer new format
  if (event_module) {

    module = window.TEALIUM.datalayer.moduledata[event_module];

    restapi_partnumbers = module.data.restapi_partnumbers;
  }
  // page datalayer legacy format
  else {

    restapi_partnumbers = window.TEALIUM.datalayer.view.restapi_partnumbers;
  }

  if (restapi_partnumbers === undefined) {

    return callback.apply(this, [localdata]);
  }

  var ajax_calls = restapi_partnumbers.length;

  for (var i = 0, rlen = restapi_partnumbers.length; i < rlen; i++) {

    window.TEALIUM.ajax.localrequest(restapi_partnumbers[i], country, preferred_language, function(ajaxdata) {

      var page_version = "product_item_names_" + window.TEALIUM.datalayer.view.visit_language,

        preferred_version = "product_item_names_" + ajaxdata.preferred_language,

        event_module = window.TEALIUM.datalayer.event,

        module = window.TEALIUM.datalayer.moduledata[event_module];

      // already preferred language
      if (ajaxdata.ajax_status === 304) {

        // update data page datalayer new format
        if(event_module) {

          module.data[page_version] = module.data[page_version] || [];

          module.data[page_version][ajaxdata.callbackindex] = module.data.product_item_names[ajaxdata.callbackindex];
          

          //////////////////////////////////////////////////////////////////////
          
          Object.extend(localdata, module.data);
        }
        // update data page datalayer legacy format
        else {

          var pagedata = window.TEALIUM.datalayer.view;
          
          pagedata[page_version] = pagedata.product_item_names[ajaxdata.callbackindex];
          
          
          //////////////////////////////////////////////////////////////////////
          
          Object.extend(localdata, pagedata);
        }
      }
      // only update data sources on ajax success
      else if (ajaxdata.ajax_status === 200) {

        var preferred_name = ajaxdata.product_name;            

        preferred_name += " : " + ajaxdata.product_description;
        ...
        if (event_module) {
          ...
          module.data[preferred_version] = module.data[preferred_version] || [];

          module.data[preferred_version][ajaxdata.callbackindex] = preferred_name;
        ...
        // update data page datalayer legacy format
          ...
          pagedata[page_version] = pagedata[page_version] || [];

          pagedata[page_version][ajaxdata.callbackindex] = pagedata.product_item_names[ajaxdata.callbackindex];

          pagedata[preferred_version] = pagedata[preferred_version] || [];

          pagedata[preferred_version][ajaxdata.callbackindex] = preferred_name;
        ...
        window.TEALIUM.log("warn", "product-restapi-lookup failed!");
      }
...
/*********************************************************
    ...
    Add Search field and Cart amount styling for desktop
...
var newHeader = (function () {
    ...
    function run() {
        ...
        // Do not run in mobile pages or in the checkout    
        if (document.URL.indexOf("m.ikea.com") == -1) {
            findElementAndRunFunction('menu', function(foundElement) {
                var topMenu = document.getElementById('topMenu');
                if (topMenu) {
                   try{
                        // Add css to page
                        var style  = document.createElement('style');
                        style.innerHTML = '.rightSection span#noOfCartItems{display: none;visibility: hidden;}.rightSection span#noOfCartItemsNew {border-radius: 30px;min-width: 14px;height: 14px;line-height: 14px;text-align: center;display: block;position: absolute;top: 0px;right: 14px;background-color: #fcd500;padding: 2px;font-weight: bold;color: #0269CE;}';
                        document.getElementsByTagName('head')[0].appendChild(style);
                        
                        //Add placeholder to searchbar based on current language:
                        if ((window.location + '').toLowerCase().indexOf('/fr') < 0 ) { 
                          document.getElementsByName("query")[0].placeholder="Search...";
                        } else {
                          document.getElementsByName("query")[0].placeholder="Rechercher...";
                        }
                       
                        //remove Parenthesis
                        var timeoutLoad = 1000/25;
                        var runParenthesisRemovalLoad = setInterval(function(){
                            var cartItems = document.getElementById("noOfCartItems");
                            if (cartItems){
                              if (cartItems.innerHTML.indexOf("(") > -1) {
                                window.clearInterval(runParenthesisRemovalLoad);
                                var tempcart = cartItems.innerHTML.replace(/\D/g, '');
                                var cartItemsNew = document.getElementById("noOfCartItemsNew");                            
                                if (cartItemsNew){                                    
                                    cartItems.innerHTML = tempcart;
                                    cartItemsNew.innerHTML = tempcart;
                                } else {               
                                    cartItems.innerHTML = tempcart;                    
                                    var cartItemsNew = document.createElement("span");
                                    cartItemsNew.id = "noOfCartItemsNew";                                   
                                    cartItemsNew.innerHTML = tempcart;
                                    cartItems.parentNode.insertBefore(cartItemsNew, cartItems.nextSibling);
                                }
                              }
                            }
                            timeoutLoad--;
                            if (timeoutLoad < 0) window.clearInterval(runParenthesisRemovalLoad);   
                        }, 25);

                        //Remove parenthesis every time user clicks
                        document.onclick = function() {
                            setTimeout(function(){
                                var timeout = 1000/10;
                                var runParenthesisRemoval = setInterval(function(){
                                    var cartItems = document.getElementById("noOfCartItems");
                                    if (cartItems){
                                      if (cartItems.innerHTML.indexOf("(") > -1) {
                                        var tempcart = cartItems.innerHTML.replace(/\D/g, '');
                                        window.clearInterval(runParenthesisRemoval);
                                        var cartItemsNew = document.getElementById("noOfCartItemsNew");
                                        if (cartItemsNew){                                            
                                            cartItems.innerHTML = tempcart;
                                            cartItemsNew.innerHTML = tempcart;
                                    ...
                                    timeout--;
                                    if (timeout < 0) window.clearInterval(runParenthesisRemoval);   
                                }, 10);
                            }, 300);
                        };
                        ...
                        var searchFieldParent = searchField.parentNode;
                        searchFieldParent.setAttribute("style", "padding-left:0;");
                        searchField.setAttribute("style", "padding:6px 30px 6px 10px; width: 320px; ");

                        //New search button
                        var searchButton = document.getElementById("lnkSearchBtnHeader");
                        searchButton.innerHTML = '<input id="lnkSearchBtnHeaderInput" type="submit" value="">';
                        var searchButtonInput = document.getElementById("lnkSearchBtnHeaderInput");
                        searchButtonInput.setAttribute("style", "background: url('http://m.ikea.com/irmw-resources/img/ikea_search2_small.png') no-repeat ; background-size: 26px 26px;height: 26px;width: 26px;margin: 3px 0px 0px -35px;border: none;cursor: pointer;");
                    } catch(err){}
                }
            });
        }
    ...
    function findElementAndRunFunction(elementIdToFind, callback) {     
        // Run until element id is found in the DOM.
        var run = setInterval(function() {
            try {
                var foundElement = document.getElementById(elementIdToFind);

                var footerId = 'footer';
                var refElement = document.getElementById(footerId);

                // If searched element is found, stop interval and run function.
                if (foundElement) {
                    window.clearInterval(run); // Stop interval.
                    callback(foundElement); // Run function.
                }

                // Stop interval if page footer is found but not searched element.          
                if (document.readyState === 'complete') {
                    window.clearInterval(run); // Stop interval.
                }
            } catch(err){window.clearInterval(run);}         
        }, 25);
    }
...
if (typeof utag == "undefined" && !utag_condload) {
  ...
  utag.cfg = {
    ...
    path: "//tags.tiqcdn.com/utag/ikea/ca-main/prod/",
    utid: "ikea/ca-main/201612080859"
  };
  utag.cfg.v = utag.cfg.template + "201612291302";
  utag.cond={187:0,203:0,207:0,216:0,219:0,220:0,235:0,236:0,240:0};
...
case '187':try{c[187]|=(d['page_type'].toString().toLowerCase()=='checkout > step4'.toLowerCase())}catch(e){utag.DB(e)}; break;
case '203':try{c[203]|=(d['soasta_load'].toString().toLowerCase()=='true'.toLowerCase())}catch(e){utag.DB(e)}; break;
case '207':try{c[207]|=(d['previous_page_name'].toString().toLowerCase()=='crm>create user'.toLowerCase()&&d['page_name'].toString().toLowerCase()=='crm>my profile>my profile information'.toLowerCase())}catch(e){utag.DB(e)}; break;
case '216':try{c[216]|=(typeof d['page_function']!='undefined'&&d['page_function'].toString().toLowerCase()=='custom_call'.toLowerCase()&&typeof d['ut.event']!='undefined'&&d['ut.event'].toString().toLowerCase()=='link'.toLowerCase())}catch(e){utag.DB(e)}; break;
case '219':try{c[219]|=(d['visit_language'].toString().toLowerCase()=='FR'.toLowerCase())}catch(e){utag.DB(e)}; break;
case '220':try{c[220]|=(d['visit_language'].toString().toLowerCase()=='EN'.toLowerCase())}catch(e){utag.DB(e)}; break;
case '235':try{c[235]|=(d['ut.profile'].toString().toLowerCase().indexOf('main'.toLowerCase())>-1&&d['platform_type'].toString().toLowerCase()=='production'.toLowerCase()&&d['site_platform'].toString().toLowerCase()=='click and collect'.toLowerCase()&&d['dom.url'].toString().toLowerCase().indexOf('cte'.toLowerCase())<0)}catch(e){utag.DB(e)}; break;
case '236':try{c[236]|=(d['platform_type'].toString().toLowerCase()=='development'.toLowerCase()&&d['ut.profile'].toString().toLowerCase().indexOf('main'.toLowerCase())>-1&&d['site_platform'].toString().toLowerCase()=='click and collect'.toLowerCase())}catch(e){utag.DB(e)}; break;
case '240':try{c[240]|=(d['page_type'].toString().toLowerCase()=='shopping cart'.toLowerCase())||(d['page_type'].toString().toLowerCase()=='shopping list'.toLowerCase()&&d['dom.url'].toString().toLowerCase().indexOf('cte2'.toLowerCase())>-1)}catch(e){utag.DB(e)}; break;}}};utag.pre=function() {    utag.loader.initdata();utag.pagevars();    try{utag.loader.RD(utag.data)}catch(e){utag.DB(e)};    utag.loader.loadrules();        };utag.loader.GET=function(){utag.cl={'_all_':1};utag.pre();
...
function(a,b){ try{ if(1){b['kenshoo_currency']='CAD';b['TOBEDETERMINED']='TOBEDETERMINED';try{b['kenshoo_products']=b.product_ids.join("|");}catch(e){}} } catch(e){ utag.DB(e) }  },
function(a,b){
...
function(a,b){ try{ if(b['page_name'].toString().toLowerCase()=='checkout>confirmation'.toLowerCase()){b['delivery_method']=b['cp.adgear_pick_up_point']} } catch(e){ utag.DB(e) }  },
function(a,b){
...
// check for Adobe Analytics being initialized & initiate the s.tl() function used by adobe analytics, for tracking
  ...
  utag.handler.cfg_extend=[{"alr":1,"bwq":0,"id":"98","blr":0,"end":0},{"alr":1,"bwq":0,"id":"99","blr":0,"end":0},{"alr":1,"bwq":0,"id":"100","blr":0,"end":0},{"alr":1,"bwq":0,"id":"128","blr":0,"end":0},{"alr":1,"bwq":0,"id":"101","blr":0,"end":0},{"alr":0,"bwq":0,"id":"135","blr":1,"end":0},{"alr":1,"bwq":0,"id":"137","blr":0,"end":0},{"alr":0,"bwq":0,"id":"149","blr":1,"end":0},{"alr":1,"bwq":0,"id":"162","blr":0,"end":0}];
  utag.loader.initcfg = function(){
    utag.loader.cfg={"2":{load:1,send:1,v:201609070801,wait:1,tid:20010},"6":{load:1,send:1,v:201511110911,wait:1,tid:3015},"13":{load:1,send:1,v:201511110911,wait:1,tid:20010},"10":{load:utag.cond[187],send:1,v:201511110911,wait:1,tid:11003},"23":{load:utag.cond[203],send:1,v:201601241810,wait:0,tid:13084},"25":{load:utag.cond[187],send:1,v:201611210915,wait:1,tid:7117},"37":{load:utag.cond[240],send:1,v:201612081612,wait:1,tid:20067},"28":{load:utag.cond[207],send:1,v:201604180819,wait:1,tid:20067},"34":{load:utag.cond[216],send:1,v:201607201304,wait:1,tid:19063},"38":{load:utag.cond[219],send:1,v:201611211336,wait:1,tid:20010},"39":{load:utag.cond[220],send:1,v:201611210915,wait:1,tid:20010},"40":{load:1,send:1,v:201610250808,wait:1,tid:8009},"56":{load:utag.cond[236],send:1,v:201612291302,wait:1,tid:19063},"57":{load:utag.cond[235],send:1,v:201612291302,wait:1,tid:19063}};
utag.loader.cfgsort=["2","6","13","10","23","25","37","28","34","38","39","40","56","57"];
  }
utag.loader.initcfg();
}
      ...
      try {
        ...
        if ($container) {
          ...
          if (shoppinglist[articlenumber] && url_quantity) {
            ...
            window.TEALIUM.trackEvent("list", "add_to_list", listdata);
          }
        }
      }
      catch(e) {
...
try{
if ( utag.data.page_name === 'checkout>select billing address'){

try { 

// If checkobox available set event listener to fetch value on change

document.getElementById('LS230_checkout_pup_address').addEventListener('change', function() {

var wasd = wasd || {};
wasd.eventdata = utag.data;
wasd.eventdata.site_platform = 'independent pages|ca|custom_measurements'; 
wasd.eventdata.site_level_2 = 'checkout';
wasd.eventdata.site_level_3 = 'checkout';
wasd.eventdata.site_level_4 = 'checkout';
wasd.eventdata.page_function = "custom_call";
if (LS230_checkout_pup_address.value == "" ) {wasd.eventdata.delivery_method = "home delivery";} 
else {wasd.eventdata.delivery_method = "Pick-up point: "+LS230_checkout_pup_address[parseInt(LS230_checkout_pup_address.value)+1].innerHTML }
document.cookie ="adgear_pick_up_point="+utag.data.delivery_method
utag.link(wasd.eventdata, null, [34]);
utag.data.page_function = "";
}
)

// If checkobox available and not changed from default

if (LS230_checkout_pup_address.value == "" ) {			
			var wasd = wasd || {};
			wasd.eventdata = utag.data;
			wasd.eventdata.site_platform = 'independent pages|ca|custom_measurements'; 
			wasd.eventdata.site_level_2 = 'checkout';
			wasd.eventdata.site_level_3 = 'checkout';
			wasd.eventdata.site_level_4 = 'checkout';
			wasd.eventdata.delivery_method = "home delivery";
            document.cookie ="adgear_pick_up_point="+utag.data.delivery_method;
		 	wasd.eventdata.page_function = "custom_call";
		    utag.link(wasd.eventdata, null, [34]); 
		    utag.data.page_function = "";
...
// If checkobox available and changed from default
...
if (LS230_checkout_pup_address.value != "" ) {	
			...
			wasd.eventdata.site_level_4 = 'checkout';
			wasd.eventdata.page_function = "custom_call";
			wasd.eventdata.delivery_method = "Pick-up point: "+LS230_checkout_pup_address[parseInt(LS230_checkout_pup_address.value)+1].innerHTML }
            document.cookie ="adgear_pick_up_point="+utag.data.delivery_method;
			utag.link(wasd.eventdata, null, [34]);
			utag.data.page_function = ""; 
}
			...
			// If checkobox available not available
			var wasd = wasd || {};
			wasd.eventdata = utag.data;
			wasd.eventdata.site_platform = 'independent pages|ca|custom_measurements'; 
			wasd.eventdata.site_level_2 = 'checkout';
