...
swa.calendar = function() {
	var _namespace = "swa.calendar";
	
	return {

		CalendarLinker: function(defaultMinDate, defaultMaxDate, departDateElem, returnDateElem) {
			this.minDate = defaultMinDate;
			this.maxDate = defaultMaxDate;
			this.departDate = departDateElem;
			this.returnDate = returnDateElem;

			this.updateDates = function() {
				var minDateCandidate = this.departDate.val();
				if (minDateCandidate != null && minDateCandidate != '' && minDateCandidate != 'When?') {
					this.minDate =
					swa.calendar.util.getNumDaysFromToday(new Date(swa.calendar.util.getDateFromString(minDateCandidate)));
				}
				var maxDateCandidate = this.returnDate.val();
				if (maxDateCandidate != null && maxDateCandidate != '' && maxDateCandidate != 'When?' && minDateCandidate != maxDateCandidate) {
					this.maxDate =
					swa.calendar.util.getNumDaysFromToday(new Date(swa.calendar.util.getDateFromString(maxDateCandidate)));
				}
			};
		...
		// Creates a calendar pair on given elements
		// Guarantees no overlapping dates
		// Does not allow user to enter dates manually in the text input field
		createCalendarPair: function(departElem, returnElem, defaultMinDate, defaultMaxDate, showOn, daysOffset) {
			var calendarLinker = new swa.calendar.CalendarLinker(defaultMinDate, defaultMaxDate, departElem, returnElem);

			$(departElem).datepicker({
				minDate : calendarLinker.getCurrentMinDate(),
				maxDate : calendarLinker.getCurrentMaxDate(),
				showOn : showOn || "both",
				...
				closeAtTop: false,
				beforeShow: function() {
					return { maxDate: calendarLinker.getCurrentMaxDate() };
				},
				onClose: function() {
                    if (daysOffset > 0) {
                        var departDateObj = new Date(swa.calendar.util.getDateFromString(departElem.val()));
                        var returnDateObj = new Date(swa.calendar.util.getDateFromString(returnElem.val()));
                        if (departDateObj > returnDateObj) {
                            departDateObj.setTime(departDateObj.getTime() + (daysOffset
                                    * (swa.calendar.util.MILLISECONDS_IN_ONE_DAY)));
                            returnElem.val(departDateObj.toMMDDYYYY());
                        }
                    }
                }
            });

			$(returnElem).datepicker({
				minDate : calendarLinker.getCurrentMinDate(),
				maxDate : calendarLinker.getCurrentMaxDate(),
				showOn : showOn || "both",
				buttonImage :"/images/14.2_calendar_icon.gif",
				...
				closeAtTop: false,
				beforeShow: function() {
					return { minDate: calendarLinker.getCurrentMinDate() };
				},
				onClose: function() {
                    if (daysOffset > 0) {
                        var departDateObj = new Date(swa.calendar.util.getDateFromString(departElem.val()));
                        var returnDateObj = new Date(swa.calendar.util.getDateFromString(returnElem.val()));
                        if (departDateObj > returnDateObj) {
                            if (swa.calendar.util.getNumDaysFromToday(returnDateObj) >= daysOffset) {
                                returnDateObj.setTime(returnDateObj.getTime() - (daysOffset
                                        * (swa.calendar.util.MILLISECONDS_IN_ONE_DAY)));
                            } else {
                                returnDateObj.setTime(returnDateObj.getTime()
                                        - (swa.calendar.util.getNumDaysFromToday(returnDateObj)
                                        * (swa.calendar.util.MILLISECONDS_IN_ONE_DAY)));
                            }
                            departElem.val(returnDateObj.toMMDDYYYY());
                        }
                    }
                }
            });
		...
		// Creates a calendar pair on given elements
		// Does NOT guarantee no overlapping dates
		// Allows user to enter dates manually in the text input field
		createSimpleCalendarPair: function(departElem, returnElem) {
...
/*** Helper Functions for swa.calendar ***/

swa.calendar.util = function() {
	var _namespace = "swa.calendar.util";
	...
	return {
        ...
        getNumDaysFromToday: function(date) {
			return Math.ceil((date.getTime() - new Date().getTime()) / this.MILLISECONDS_IN_ONE_DAY);
		},

		parseDateToMonth: function(dateString) {
			var monthIndex = parseInt(dateString.substring(0, 2), 10) - 1;
			var months = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
			return months[monthIndex];
		},

		getDateFromString: function(dateString) {
			var pos1 = dateString.indexOf(this.delimiter)
			var pos2 = dateString.indexOf(this.delimiter, pos1 + 1)
			var year = this.parseYearToInt(this.parseYearToString(pos2, dateString));
			var month = this.parseMonthToInt(this.parseMonthToString(pos1, dateString));
			var day = this.parseDayToInt(this.parseDayToString(pos1, pos2, dateString));
			var date = new Date();
			return date.setFullYear(year, month - 1, day);
		},

		parseDayToInt: function(dayString) {
			if (dayString.charAt(0) == "0" && dayString.length > 1)
				dayString = dayString.substring(1)
			return parseInt(dayString)
		},

		parseDayToString: function(pos1, pos2, dateString) {
			return dateString.substring(pos1 + 1, pos2);
		},

		parseYearToInt: function(yearString) {
			for (var i = 1; i <= 3; i++) {
				if (yearString.charAt(0) == "0" && yearString.length > 1)
					yearString = yearString.substring(1)
			}
			return parseInt(yearString)
		},

		parseYearToString: function(pos2, dateString) {
			return dateString.substring(pos2 + 1);
		},

		parseMonthToInt: function(monthString) {
			if (monthString.charAt(0) == "0" && monthString.length > 1)
				monthString = monthString.substring(1)
			return parseInt(monthString)
		},

		parseMonthToString: function(pos1, dateString) {
			return dateString.substring(0, pos1);
		}
...
Date.prototype.toMMDDYYYY = function() {
	...
	function doubleDigitString(str) {
		str += "";
		if (str.length == 1) {
			str = "0" + str;
		}
	...
	var monthString = doubleDigitString(this.getMonth() + 1);
	var dateString = doubleDigitString(this.getDate());
	var yearString = this.getFullYear();
	return monthString + "/" + dateString + "/" + yearString;
};

function fixDate (inputField) {
	var dates = inputField.split("/");
	if (dates.length ==3)
	{
		if (dates[0].length < 2) {
			dates[0] ="0" +dates[0];
		...
		inputField = dates[0] + "/" + dates[1]+ "/" + dates[2];
	}
...
function Datepicker() {
	...
	this.regional[''] = { // Default regional settings
		...
		closeText: 'Close <img src="/images/24.2_icon.jpg" />', // Display text for close link
		closeStatus: 'Close without change', // Status text for close link
		prevText: '<img src="/images/1.3_left_arrow.gif" alt="Previous" />', // Display text for previous month link
		prevStatus: 'Show the previous month', // Status text for previous month link
		nextText: '<img src="/images/1.3_right_arrow.gif" alt="Next" />', // Display text for next month link
