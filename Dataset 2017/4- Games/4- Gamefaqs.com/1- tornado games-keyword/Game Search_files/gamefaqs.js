...
function show_login()
{
	if($(document).width() < 500)
		...
		var box_width = "30%";
	$('#login_dialog').dialog({	resizable: false, dialogClass: "reg_dialog", closeText: "X", height: "auto", width: box_width, modal: true, open: function(){$('.ui-widget-overlay').bind('click',function(){$('#login_dialog').dialog('close');});} });
	return false;
}
...
function show_gs_video_dialog(id)
{
	...
	$('iframe.gs_video').attr('src','http://www.gamespot.com/videos/embed/'+id+'/?autoplay=1&utm_source=gamefaqs&utm_medium=partner&utm_content=video_player&utm_campaign=gamespace_video');

	if($(document).width() < 500)
		var box_width = "95%";
	else
		var box_width = "680px";
	$('.gs_video_dialog').dialog({ resizable: false, dialogClass: "reg_dialog", closeText: "X", height: "auto", width: box_width, modal: true, open: function(){$('.ui-widget-overlay').bind('click',function(){$('.gs_video_dialog').dialog('close');$('.gs_video').attr('src','');});} });
	return false;
}
