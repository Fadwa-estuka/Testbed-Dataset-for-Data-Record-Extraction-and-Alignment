(function(win,doc){ // EUROPA 2013 "RESPONSIVE DESIGN" - 2.4.9
...
// CORPORATE
	...
	mobileRun	:function(){
		...
		// search form
		if(sf){var f=sf;if(f){f.className="searchform search-on";var i=f.getElementsByTagName('input'),s=i[1],i=i[0];if (i.value!=""){f.className="searchform"};function hide(){f.className="searchform";}function show(){if(i.value==""&&!f.isFocus){f.className="searchform search-on";}}function foc(){f.isFocus=true;hide();}function blu(){f.isFocus=false;show();}i.onmouseout=show;i.onfocus=foc;i.onblur=blu;}}
...
win.webservice={ // Retrieve any translations of any documents and showing inside a popup.
	...
	popup			:function(srcElm,coverage){var e=srcElm,span=e.parentNode,wsUrl=(span)?span.u:null;if(span.tagName!="SPAN"){span=document.createElement("span");span.className="ws-popup";_$.wrap(e,span);if(coverage){wsUrl=span.u=coverage;}else{var p=webservice.prevLink(span,"A");wsUrl=span.u="/cgi-bin/coverage/coverage?url="+encodeURIComponent(decodeURIComponent(p.href));}}
		...
		function getList(xml){var h,s,t,v='',p,b,r,a,i,j,e,z,n,l='';z=xml.getElementsByTagName("message");d=xml.getElementsByTagName("document");p=d.length;n=z.length;c=false;k=doc.lang;for(i=0;i<p;i++){b=d[i];r=b.getAttribute("lang");a=b.getAttribute("label");t=b.getAttribute("type");e=b.getAttribute("href");s=a.split("(")[0];l +='<a class="lang" href="'+e+'" hreflang="'+r+'" lang="'+r+'" title="'+a+'"><span class="off-screen">'+s+' (</span>'+r+'<span class="off-screen">)</span></a> ';}return {lst:l,nbr:n,cnt:p,error:z};}
